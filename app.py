...
# 1. Configuração visual da página (DEVE SER A PRIMEIRA LINHA)
st.set_page_config(page_title="Assistente PEM Sudeste", page_icon="🌊", layout="wide")

# 2. Injetando CSS Personalizado (A Magia do Design)
st.markdown("""
<style>
/* Cor de fundo da tela (um azul oceano bem clarinho e elegante) */
.stApp {
    background-color: #F0F8FF; 
}
/* Cor dos títulos (Azul marinho profundo) */
h1, h2, h3 {
    color: #004B87 !important;
}
/* Estilizando as caixas de texto e fundo branco para leitura */
...
# 3. Imagem de Capa (Banner do Mar)
# Usando uma imagem em alta resolução de ondas do mar
st.image("https://images.unsplash.com/photo-1505118380757-91f5f5632de0?q=80&w=2000&auto=format&fit=crop", use_container_width=True)
...
st.divider() # Linha visual para separar o cabeçalho

# 4. Conectando a Chave Secreta do Streamlit
try:
    genai.configure(api_key=st.secrets["GEMINI_API_KEY"])
except Exception as e:
    st.error("⚠️ Chave de API não encontrada nos Secrets do Streamlit.")

# 5. Criando a Personalidade: O Acadêmico Rigoroso
...
# 7. Criando as duas abas
aba1, aba2 = st.tabs(["🔮 Consulta Técnica (Chat)", "⚖️ Comparador de Zoneamento"])

# --- ABA 1: O ORÁCULO ---
with aba1:
    st.header("Consulta Técnica Aprofundada")
    st.info("💡 **Dica:** Pergunte sobre regulamentações de portos, áreas de proteção ou regras de pesca.")
    ...
    if pergunta:
        ...
        with st.spinner("Analisando bases técnicas e jurídicas do oceano..."):
...
# --- ABA 2: O COMPARADOR ---
with aba2:
    st.header("Análise Comparativa Regional")
    st.write("Selecione os temas para gerar um relatório técnico comparativo entre o Sul e o Nordeste.")
